 ...
 *
 * Lived outside src/db so client components could import the list without
 * dragging mysql2 (and `server-only`) into the browser bundle. Same
 * reasoning, one level further out: a client that is not the web app at all
 * still has to name these statuses, and there is only one right list.
 */
